...
def scan(assets_dir: str, body: str, blank_sd: float = BLANK_SD) -> dict:
    ...
    for f in names:
        ...
        read += 1
        if m["sd"] < blank_sd:
            blank.append({"name": f, "width": m["width"], "height": m["height"], "extrema": m["extrema"], "sd": m["sd"],
...
def main(argv: list[str]) -> int:
    ...
    for bundle in argv[1:]:
        ...
        print("%s: assets=%d read=%d unread=%d blank=%d referenced=%d (sd < %s) %s" % (
            bundle, r["assets"], r["read"], r["unread_total"], r["blank_total"], r["blank_referenced"], r["blank_sd"],
            "; ".join("%s %dx%d ext %d/%d sd %.2f %s" % (b["name"], b["width"], b["height"], b["extrema"][0], b["extrema"][1], b["sd"],
                                                           "REF" if b["referenced"] else "unref") for b in r["blank"])))
    return red
