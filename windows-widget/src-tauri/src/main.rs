...
fn vault_check(state: State<AppState>) -> Result<vault::VaultStatus, String> {
    ...
    let dir = state.config.lock().map_err(|_| "lock poisoned".to_string())?.vault_library_dir.clone();
    Ok(vault::check(&dir))
}
#[tauri::command]
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![list_portals, send_to_portal, fetch_file_status, vault_check, vault_pull])
