...
> **S117 update (2026-09-06):** the REGISTER gate is now **ARMED** too (Rab's "arm REGISTER" on the S116 sign line): an OPEN-TASKS.md untouched since the pin exits 1; a session that struck and added nothing writes one dated line in the register saying so, because the gate reads the file, not the closeout. Two selftest shapes (untouched → 1, written → 0; no register → UNREAD as before); muster selftest 73/73.
