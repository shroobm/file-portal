...
MECHANISM = {
    ...
    "S15 THE-INSTRUMENT-MANUFACTURES-THE-FINDING": (  # SYM-141 (S209 E13): the ladder's orphan backslash — added below
        ...
        "artefact does not?",
        ["SYM-067", "SYM-076", "SYM-095", "SYM-097", "SYM-138", "SYM-141", "SYM-145", "SYM-146", "SYM-147", "SYM-157", "SYM-167", "SYM-168", "SYM-169", "SYM-170", "SYM-171"],  # 167–171 (S211 E3, the accounting): the witness's dehyphenation, the dot leaders, the tag strip, the scan-lane agreement counts, the furniture inside a figure box — the instruments' own findings  # 138 (S209): the analyst audit's near-exact window reads a MERGED stacked header as a 36-word omission — the improvement fails the document; 141: the ladder's orphan backslash (an escaped asterisk) reads as a lost word; 145 (Codex MSG-CDX-0085): phantom rulings counted as columns lost; 146: ligature glyphs counted as inventions; 147 (Codex MSG-CDX-0085): a TableGroup's moved row the measure never read; 157 (S210 E1, Desjardins): the degeneration detector's space-free fallback reads a rule line of 172 escaped underscores as a loop (trigram `\ _ \` ×171)
    ),
