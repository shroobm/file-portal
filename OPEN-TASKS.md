...
| A4 | **P-1 variable 1: HOST** | in-converter at `:998` (route to region-level coverage, extends card-lock) ~or~ out-of-band desktop re-score (zero lock cost, back-fills the whole corpus) | *stated as his, deliberately not recommended away* | `docs/37` §3.6, `docs/41` §2 |
...
|---|---|---|
| A18 | **Wire the SYM-050 repair.** `true_page = id + 1 − 200×(id÷400)`, confirmed four independent ways at S106, **is not in the shipped tool** — it still reports IV uncovered 239 / coverage 0.1115 instead of 49 / 0.8178 | `Observed 2026-08-22` — S106 §12 records it unwired; SYM-050 status `OPEN` |
...
| # | Item | Source |
|---|---|---|
| B17 | **Re-measure throughput with the A-B-A control on an idle card.** The instrument is built and validated; a clean run is what is missing. **Do not quote 0.77×, 0.62×, 0.95× or 0.81× — all four are withdrawn** | S82 §18.2–3 |
| B18 | **Raise `n` to ≥ 20–30** before any ratio is quoted. 5–8 of 266 chunks is `Inferred` about the book; the no-think arm runs ~5 s/chunk, so n=30 is ~3 min of card time | S80 §18.3, S81 §18.2 |
...
|---|---|---|---|
| D1 | **Codex** | Four asks, standing since S99: adopt a **model trailer** on its commits · **claim its S97 sections** per `coordination/authorship.md` · write **`docs/43` §3** in its own words · **leave a relay entry back** | **Never collected.** `Observed 2026-08-22`: `relay.md` holds **3 entries, all `⟨from: Fable⟩ → ⟨to: Codex⟩`, zero returned.** Stamps 15:0, commit trailers 49:0. `docs/43` §3 is still an empty scaffold. The relay's consumption rule **has never once been executable** |
| D2 | **Codex** | The one probe Fable could not run: **re-measure P-1 on a CLEAN post-S60 bundle** independently and say whether SYM-050 holds | Not run by Codex. S106 did it Desktop-side via a *repaired map*, because **no clean large born-digital bundle exists** (4 born-digital books in the corpus; the only large one is the only chunked and only poisoned one). An independent re-run is still worth more |
| D3 | **The next Circle** | **`docs/45` §6, the bequest — 5 items.** (1) re-check `docs/32` §6's prediction (2) *does Family 1 have a check yet — what compares a claim to the probe that produced it?* (3) re-measure P-1 clean (4) P-2 now has multiple specimens across **different** failure modes (5) the explicit UNREAD list | S106's Circle collected part of it and **refuted item 4's own framing** (see §E). **§6.2 remains fully open: nothing compares a claim to its probe.** S106 proved it again by shipping a rule it had not measured |
...
| **P-1** figure/vector completeness | **BUILT S102, vetoed S104, re-measured S106 — and still UNWIRED.** Its host variable (A4) is unsigned, the SYM-050 map repair (A18) is not in the shipped tool, and two of its constants are inert (B14) |
...
- **The exporter supersede flow** — BUILT at S43 (`bd02fc0`), live-fired S50/S56. *The Desktop
  half of the seam is what remains* (D7).
