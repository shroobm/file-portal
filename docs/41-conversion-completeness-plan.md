...
## Addendum S157 (2026-09-15) — what the long sitting did to P-1, P-2 and P-3

> **P-1 (E25): the instrument's blind spot since S104 is fixed.** Zero-area paths were dropped before clustering (SYM-049: a connector never joined the boxes it connects — the Book of Models' p.34 and p.78 invisible since S105). Now a zero-area path is clustered when it is ANCHORED (within the gap of a positive-area path or a perpendicular line — a stack of bare rules still stays out), and the table veto is DISQUALIFIED for a cluster with ≥ 3 sizable curves/diagonals (S105's "0-for-11" class: a Watt engine is not a 16×18 table — measured over every (cluster, vetoing table) pair: the Book of Models 14 of 15 non-rectilinear, Investment Valuation 0 of 144). Three levers, each ceiling restoring the S104 behaviour; two report counters. Measured on every anchor before and after: the Book of Models gains exactly p.34 and p.78 as uncovered (1 → 3), figure pages 57 → 71; no other book moves a page. Still report-only and unwired (variable 1 remains Rab's). Residue: a rectilinear chart (Cyb p.40) still reads as a table.
>
> **P-1 re-measured on CLEAN bundles (E24, docs/45 §6.3's ask):** the IV University Edition (SYM-050 not detected) reads 1,377 pages · 293 figure pages · **76 uncovered** · 0.7406, both copies identical; the 4e (2025) bundle 49 under the in-memory offset repair (239 unrepaired — the S104 number). On that bundle the ASSET ids are doubled and the S106 formula holds (asset 912 is page 513), while the SPAN anchors are plain — two namespaces (E30).
>
> **P-2 (E24, E30): the specimen count has an enumeration.** Two-reader Sonnet fleets with planted controls over the rendered uncovered pages: the University Edition **76 → 11 lost figures** (5 absent: FIGURE 9.3, 11.4, 11.7, 12.3 and an uncaptioned event-window timeline; 6 flattened into tables or prose: 3.1, 7.1, 9.1, 9.2, 20.12+20.13, 24.1) and 65 non-figures; the 4e (2025) **49 → 10** (nine of them the same figures; 15.1's spreadsheet screenshot the tenth). "Flattened" — a labelled-box diagram or a screenshot OCR'd into a pipe table or headed prose — is the class §12's third framing asks about ("is it even wrong?"); its disposition (A5) stays Rab's.
>
> **P-3-shaped (E28, E31): a page-level text-loss locator exists, private and report-only** — `measurement/probes/prose_loss.py`: every prose line of every source page searched in the bundle by three 8-word windows (ligatures folded, link targets and real tags stripped); per page and per book, with the missing lines; `--attribute-runs` gives the survival audit's analyst-stage omission runs (page null) their page and line bbox. Not RETAS/FCA (no alignment, no character accuracy) — a locator, which is what P-3 said the text-loss problem needed first. What it found: the bundles converted before the S140 acceptor lost whole sections at the analyst stage (300–600 words; the IV University Edition's two conversions lost different ones), Marker's own largest loss on that book 154 words; the acceptor reverts the class (`edit_whitelist_selftest` case 29); the pre-acceptor shelf needs re-conversion (register `held-books/reconvert-four`).
