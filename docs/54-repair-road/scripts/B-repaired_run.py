...
json.dump(out, open("repaired_result.json","w",encoding="utf-8"), indent=2)
print(json.dumps(out, indent=2)[:3000])

# also compute overall verdict using manifest's historical analyst block
manifest = json.load(open("manifest.json", encoding="utf-8"))
analyst_block = manifest["fidelity"]["analyst"]
verdict = fa.compute_verdict(block, analyst_block)
print("OVERALL VERDICT (repaired convert + historical analyst):", verdict)

# also verdict for convert-alone (no analyst)
verdict_convert_alone = fa.compute_verdict(block, None)
print("CONVERT-ALONE VERDICT:", verdict_convert_alone)
