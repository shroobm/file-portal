...
### 10.11 After the Circle (S159 E5, appended 03:1xZ) — one more correction, and the pointer this section owed

**Read this before citing §2–§7.** §8, §9 and §10 correct sentences that stand above them unmarked (append-only): §4.1's "the loss is Marker's" (withdrawn, §10.2); §4.3's OCR-classifier row (one of four page-level gates, §10.3); §4.5's neighbouring rows; §5's K and I rows (rewritten and split, §10.3–10.4) and its C-filed SYM-067/076/066/057 (W, §10.1); §2's four-level key (six levels, §10.1). A section number cited alone is the old text; this document's current reading is §10's.

**§10.6's sentence on SYM-049 is wrong and withdrawn.** It said the row's "Neither has an asset in the bundle" was false at page level because `_page_34_*` and `_page_78_*` assets exist. Marker's asset page ids are ZERO-INDEXED and the audit's pages are 1-based (`figure_coverage.py:720-735`: `per_page[int(m.group(1)) + 1]`); the assets named `_page_34_` are the row's own p.35 control ("p35 carries a near-identical arrow diagram, it DOES ship and IS flagged") and `_page_78_` are p.79's. No asset exists for the row's p.34 and p.78; the row stands. The error was a lane's (S159 E2, D-05) reading of a filename digit as a page, carried into §10.6 by the lane that should have counted; found by the Circle's C2 lane (S159 E5) against the E2 lane — a conflict the lane resolved by reading the code. SYM-049 stays **W** (the instrument's zero-area drop), as §10.6 filed it.

### 10.12 After the second Circle (S162 E3, appended 2026-09-16 19:2xZ) — six corrections the cells' lane found

**Read with §10.11's pointer: §2–§7 stand as written and this section corrects them.** The second Circle (S162; its record `docs/63`) put §4.2, §4.4 and §4.6–§4.9's numbered cells against docs/34 and the tag law. The lane's own re-check confirmed these six (C3-S1, C3-01, C3-07/C3-S2, C3-05, C3-S3, C3-08):

1. **"Beer" is bound (§3.3 owed it).** §4.2 row 1, §4.3 and §4.4 row 1 say "Beer" for the anchor that §4.1, §10.2 and §10.9 call **Diagnosing** — *Diagnosing the System for Organizations* (Stafford Beer, 1985), the S146 E8 dry run (184 pages, 868.4 s, 4.72 s/page, the sidecar 8,321 MB committed through recognition at batch 8; `sessions/JOURNAL-autonomy-2026-09-13.md` S146 E8). It is never *Brain of the Firm*, the other Stafford Beer on the shelf (the degeneration specimen, docs/15 §9). From here "Beer" alone is not written; the binding row of §3.3 reads: **Beer** → Diagnosing (the anchor) | Brain of the Firm (the specimen) | the rule: name the book.
2. **§4.2's cite of SYM-132 is wrong.** The 8,321–8,327 MB figure is S146 E8's (the JOURNAL; `OPEN-TASKS.md`'s `converter/ceiling-crawl-triggers-the-ladder` row); SYM-132's row is a different episode — the 9,515 of 10,240 MiB death and its re-run at 5.6 GB. The number stands; the pointer is corrected to S146 E8.
3. **§4.7 rows 1 and 2 say "pre-acceptor"; §10.5 retracted the word** ("pre-survival-gate"). §10.5 is the reading; the rows carry no inline pointer by the append-only rule — this line is the pointer.
4. **§4.6 row 1's "IV 4e … 10 real losses after a 49-page adjudication (E30)" and §10.10's "five on the 4e" are different denominators:** §4.6 counts the 4e's real losses among its 49 adjudicated uncovered pages (10); §10.10 counts the FLATTENED figures among those ten (5 of 10). Both stand; neither corrects the other; the relationship was never stated on the page until here.
5. **§4.8 row 1's "survival 0.69–0.99 by book" is the pre-§10.2 framing.** The range is a min–max spread of `doc_survival` (window containment against the witness); its deficit is disagreement, of which loss is one cause among nine (§10.2, §10.9). Read the range as agreement, never as loss.
6. **§4.7 row 2's "669 hyphen joins" was corrected to 670 in §8.2** (S158; `sessions/S140` F4). Cited here so the pointer sits beside the cell's own section.

WITHSTOOD (C3, Observed at their receipts): Gemini Flash's pacing constants (§4.2 row 5); the batch-16 peaks and Diagnosing's 9.5 GB (§4.4 row 1); the frame-veto calibration (§4.6 row 2); the pre-acceptor missing-line percentages (§4.7 row 1, the numbers); the Tesseract gauge (§4.8 row 3); the vault gate's refusal since S146 E6 (§4.9 row 1) — the lane inventoried 28 numbered cells across the six sub-sections: of its ten real seeded cells 6 withstood, of its eleven open-sweep findings 8 withstood; the lane's residue names what it could not reach (the 8,327 upper bound of the sidecar range — the private `wddm_sampler.ps1` evidence; docs/18's S51 attribution of the ~18 % overhead).
