...
### Linux (Claude Code)
- [x] *(verified live 2026-07-10: real version output in CLAUDE_README Session Log)* **Install engines.** `pymupdf4llm` goes in `linux-converter/.venv` (it's in `requirements.txt` — the service runs from that venv, so a system-wide `--break-system-packages` install would be invisible to it); `sudo pacman -S tesseract tesseract-data-eng pandoc-cli` (Arch packages pandoc as `pandoc-cli`). **Critical:** `import pymupdf.layout` **before** `import pymupdf4llm` or auto-OCR silently never fires on image-only pages — encoded as real import order in `converter/engines.py`.
- [x] *(verified live 2026-07-10: three converter.log lines naming the engine per extension; pandoc emits `-t gfm`, media flattened into assets/)* **Dispatch by extension (format-as-set).** `.pdf`/`.epub` → PyMuPDF4LLM; `.docx` → Pandoc (`pandoc -f docx -t markdown --extract-media=<assets>`). Each handler owns its membership by extension. **Do not** give docx its own tile — the extension is a free, certain test.
- [x] *(verified live 2026-07-10 — NOTE: in pymupdf4llm 1.28 layout mode, OCR is need-based in every lane and `force_ocr=True` maps to FORCE_KEEP_OLD, keeping a bad prior OCR layer; the shipped lanes are Clean=`SELECT_KEEP_OLD`, Scan=`FORCE_DROP_OLD`+`ocr_dpi=300`. The membership test is the chars_per_page probe in `converter/engines.py`.)* **Two lanes.**
...
- [x] *(verified live 2026-07-10: image embed resolved by ls; layout mode handles write_images fine in 1.28)* **Images → bundle.** `to_markdown(..., write_images=True, image_path="<bundle>/assets", dpi=150)` (layout mode off for image extraction). Rewrite `![](assets/..)` → Obsidian `![[..]]`. Output a **bundle folder** (markdown + assets), never a bare file.
