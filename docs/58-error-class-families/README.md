...
**Appended 2026-09-16 (S168 E1).** The library's own smoke (`library_smoke.py --public`, its first whole run since S139) read `error_families.py --selftest` RED, 11/13: two rows had been filed without a family — **SYM-133** (S157 E15, the table layer rewriting a digit column into a word under an invariant that admits it) unplaced, and **ERR-2026-09-15-109**'s class `EDIT-UNDER-EXECUTION` (S157 E24, an edit to a running bash script re-read by byte offset) unknown to every METHOD family. Placed: SYM-133 in **S16 GUARD-COARSER-THAN-THE-DEFECT** (the guard cannot see a label's identity, only that the letters fit); `EDIT-UNDER-EXECUTION` in **M8 AN-INERT-ACT-REACHED-A-WIDER-SCOPE** (an edit is inert until the process reading the file reaches the edited bytes). §2's M8 row reads two classes; it is three now. The census at the append: `ERR 109/109 · SYM 133/133`, exit 0; the selftest 13/13. The law this doc states — place before filing — was broken twice by the lane that wrote it, in one sitting (S157), and caught by the smoke the same lane had not run in twenty-nine sittings.
