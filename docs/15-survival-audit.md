...
### 9.8 Re-measured on this corpus (appended 2026-09-17, S179 E1 — a reading; the thresholds unchanged, his) — **renumbered from 9.5 on 2026-09-24 (S212, sign-sheet item 1, his word): it collided with §9.5 at the top of this document, the J32-B calibration pin. 9.8 is the next free number in this appended series; 9.6 and 9.7 below keep theirs, because nothing is renumbered that does not have to be.**
...
**The numbers (this corpus, the current rule):** flagged conversions 14 → LOOP **11** · STRUCTURE **3** (all three one book's); by distinct book 6 flagged → LOOP 5 (Diagnosing, Zero-to-One pre-repair, Investment Valuation University, Equity Research, Brain of the Firm) · STRUCTURE 1 (Ashby). **The false-alarm rate: 3 of 14 conversions (21 %), 1 of 6 books (17 %)** — **CORRECTED 2026-09-24 (S212, sign-sheet item 1, his word; measured S211 E7): the real rate is 0 OF 14 CONVERSIONS AND 0 OF 6 BOOKS.** All eight flagged blocks were read AT THEIR PAGES — four on the STRUCTURE side, where being right would have supported the exemption, and four on the LOOP side — and eight are loops. Ashby settles his own case on p.90 without any judgement from us: the book prints *"the protocol (of 50 transitions)"* — fifty letters, printed once — against a 1,975-character block. The three 'false alarms' counted above are that one book's protocols, and they are not false. **The sentence above is left standing because it is what was believed on 2026-09-17; this is a correction appended to it, not a rewrite of it.** — every false alarm the same shape: a block of one-letter tokens (a symbol protocol, an exercise), which the trigram count reads as a stutter because the alphabet is two letters. The six historical false alarms (tables) are already remedied by §9.3 and do not fire under the current rule. **The zero-FP claim (§9.1's "calibrated on the vaulted corpus") does not hold on this corpus; it holds on prose** — every LOOP verdict above is a real loop, and the one STRUCTURE class is not prose.
...
### 9.7 What the fidelity block names beside the verdict (appended 2026-09-17, S188 E6/E7 — attribution and a flag; the gates unchanged)

**`verdict_phase` (S175) and `verdict_phases` (S188 E6).** `verdict_with_phase` returns the verdict and the ONE phase that decided it in the signed order of §12 — the analyst near-exact gate first, then degeneration, then the localisers — so a book whose convert phase degenerated AND whose analyst audit fell under the gate read `fail · analyst` alone (Automate the Boring Stuff, S187: the Marker loop a bench reader looks for first is the convert phase's). `fail_phases` names EVERY phase whose fail signal fired, in pipeline order — `["convert"]`, `["analyst"]`, `["convert", "analyst"]`, `[]` on a pass or a flag (a flag is a localiser, not a fail) — and `build_fidelity_block` writes it as `verdict_phases` beside the pair; the three hand-set sites in `convert_and_ship.py` (the analyst audit's two branches, the `--reaudit` path) write it too. The verdict and the deciding phase are unchanged; this is attribution. On a re-audited bundle `fidelity.convert` / `fidelity.analyst` are HISTORY and the re-audit's blocks live under `fidelity.final`; the verdict and the phases follow `final`. Tripwires: `convert_and_ship_selftest.py` T27 (f)–(h).

**`manifest["blank_assets"]` (S188 E7; SYM-053's tripwire, report-only).** `blank_assets.scan()` reads every image under the bundle's `assets/` — grayscale standard deviation under `BLANK_SD = 1.0` is paper (Beer p129's `_page_128_Picture_15.jpeg`, 511×70 at 240/240, reads 0.00; a real crop reads tens) — and whether the pre-analyst body references it: a referenced blank is a page that LOOKS covered while its drawing is gone. Recorded as counts and a capped list (`blank_shown_cap` beside `blank_total`, NUM-3), `mode: "flag"`, beside `latex_structure`; unreadable images are UNREAD, never blank. **The verdict does not read it** — §12's two fail signals and its localisers are as they were; what a blank crop DOES (a page flag, a re-crop, the bench) is a gate change and Rab's. Measured first on the corpus: two distinct blank assets in ~2,300 across the anchor and held bundles, both referenced. Tripwires: `blank_assets_selftest.py` (the S108 shape positive; a gradient negative; the lever's own negative control; a corrupt file UNREAD; the CLI's exits) and T29 in the converter suite; the suite is on `close.sh`'s runner list.
