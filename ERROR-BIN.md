...
| ERR-2026-09-13-102 | STALE-GROUND | S144 E3a, the launch of the four re-conversions on Rab's word (09:12Z) | that a re-converted book whose audit fails would HOLD with a sidecar, as the S142 triage wrote and as the 08-25 and 09-12 holds had done | the converter ships every verdict unless `audit-mode.txt` reads `enforce`; it had read `report` since 09-12 02:59:52Z (no record of the writer); Ashby (verdict fail, degeneration) shipped, and the exporter's first-ingest path — no verdict guard of its own — put it in the vault (54b471d7, 11:31:10Z) | the ThinkPad's `EXPORTED` line read at 11:31Z against the manifest's `verdict fail`; Beer's run killed by hand two minutes in, the queue stopped, the three decisions put to Rab | a launch that can ship reads the lever that decides shipping FIRST (`audit-mode.txt` beside `.gpu-lock`, `nvidia-smi`, `query user`); "fails hold" is a lever's state, not a law; the open card prints the mode (SYM-131) |
