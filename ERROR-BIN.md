...
**Extended a twelfth time 2026-08-28 by the Codex lane:** rows **039–043**, four existing
classes recurring during close. Row 039 is the consequential one: the first mechanical close
waited about fourteen minutes in an interactive credential helper because the later HTTP timeout
did not bound the earlier credential read. Rows 040–043 were failed-closed audit harnesses or an
immediately corrected count; none produced a repository mutation or durable product claim.
§C.18 appends the current 43-row reading without rewriting any prior checkpoint.

**Extended a thirteenth time 2026-08-28 by the Codex lane:** rows **044–045**, two same-close
recurrences. Row 044 repeated ERR-040 minutes after that rule was filed; Git refused before any
claim. Row 045 caught an undeclared one-byte section-boundary difference between two otherwise
agreeing immutability probes before the closeout committed. §C.19 preserves both facts.
...
| **HARNESS-MISUSE** | The command returns 0, or the harness fails on its own stale wrapper. The tooling result is then about launch mechanics or a test double rather than the runtime behavior under test | A backgrounded command must be the whole job, not a launcher for another background job. Verify backups before restore, scope repository identity explicitly, assemble argument vectors exactly, and make monkeypatches forward the complete runtime call contract before treating suite output as product evidence | 007, 008, 033, 040, 043, 044 |
| **QUOTING** | A script dies on its own delimiters: backslashes, nested quotes, a trailing `\'`, a regex mangled in transit. Cheap when it crashes; expensive when it half-writes | Anything containing backslashes or nested quotes goes in a FILE via `Write`, never a heredoc or shell-shaped one-liner | 009, 013, 029, 042 |
...
| **DENOMINATOR** *(added 2026-08-27, row 012)* | Two totals appear in one session, or one count silently changes its population or byte boundary. Both can look plausible while answering different questions | `docs/34`: every measured number names its **numerator, denominator and conditions**. Two totals of different populations must never appear without their populations stated; occurrence, distinct-value, and delimiter-inclusive byte counts are separate measurements | 012, 041, 045 |
...
| **BOUNDARY-OMISSION** *(added 2026-08-28, row 022)* | Every required field is present, correctly typed, digested and confirmed, or the external call has a timeout—but a load-bearing predicate, equality, or earlier blocking boundary is absent | Test every property the protocol exists to establish, including falsifying boundaries and equality bindings to canonical evidence. Bound every external interaction, not only the last one; final readings must differ; sidecar chronology must equal its relay source; every receipt proving completion must precede the terminal act | 022, 034, 036, 039 |
...
| ERR-2026-08-28-039 | BOUNDARY-OMISSION | first S111 `close.sh e0c371a` invocation | Launched the signed close normally and treated its CI probe as bounded because the GitHub `curl` carries `--max-time 25` | Before `curl`, `git credential fill` launched `git-askpass` and waited interactively for about fourteen minutes. The HTTP timeout could never fire. Rab's “what's the hang?” prompted an exact process-tree read; the attempt was interrupted with no repository mutation and rerun with `GIT_TERMINAL_PROMPT=0`, `GCM_INTERACTIVE=Never`, and `GIT_ASKPASS=/usr/bin/false`. It completed and honestly printed CI `UNREAD — no stored credential` | Rab asked about the hang; Codex then traced the exact `bash → git → git-askpass` descendants and rejected elapsed time as evidence of progress | A bounded probe must bound **every** external interaction in execution order. A timeout on the network call does not bound an earlier credential helper; close invocations must prohibit interactive credential prompting or wrap credential acquisition itself |
| ERR-2026-08-28-040 | HARNESS-MISUSE | memory-repository close-ground probes | The first read-only Git batches omitted the required per-command `safe.directory` scope | Git refused both batches as dubious ownership, so they produced no evidence. The probes were rerun with exact per-repository `-c safe.directory=<path>` and no global configuration change | Git's refusal, before any claim or write | Repository identity is part of the harness. On this machine, every memory-repository Git probe carries an exact per-command safe-directory scope; a failed ownership check is UNREAD, never evidence |
| ERR-2026-08-28-041 | DENOMINATOR | pre-push absolute-path scan | The close-contract auditor first summarized the result as four added absolute paths | The scan contained **five added occurrences representing three distinct values**. The auditor corrected the count immediately before it entered the close record | Recounting the original output and separating occurrences from distinct path values | Every count names its population and predicate. “Occurrences” and “distinct values” are separate denominators and must never share one number |
| ERR-2026-08-28-042 | QUOTING | first raw-byte §1 comparison harness | A Python `-c` one-liner was sent through PowerShell to extract and hash the section | PowerShell rejected the shaped string before evidence was produced. The auditor moved the byte logic out of the shell-shaped expression and reran successfully | The parser error; no claim or file mutation preceded recovery | Byte probes with nested quoting belong in a file or a native argument-safe form. A shell parser failure is UNREAD, not a partial comparison |
| ERR-2026-08-28-043 | HARNESS-MISUSE | second raw-byte §1 comparison harness | A first-pass .NET invocation assembled Git's argument list as though one string were an exact argv | The Git read failed before evidence. A corrected raw-byte probe then measured opening, HEAD, and disk as 94 identical bytes and a one-byte mutation as different | The command failure, before any claim or write | A process harness must pass the exact argument vector the child program expects. Validate launch shape with a cheap read before trusting the substantive measurement |
| ERR-2026-08-28-044 | HARNESS-MISUSE | main-lane post-authorship §1 byte verifier | Minutes after filing ERR-040's exact safe-directory rule, Codex wrote a new Git subprocess harness without that per-command scope | Git refused the opening-blob read as dubious ownership. No comparison or claim was produced. The script was patched to inject `-c safe.directory=<exact worktree>` before every Git call and then succeeded | Git's refusal; the just-filed ERROR-BIN rule did not prevent the recurrence | A rule written during the same close still has to be applied to the next command. Centralize repository identity in the harness constructor so a caller cannot forget it |
| ERR-2026-08-28-045 | DENOMINATOR | §1 raw-byte immutability evidence | The draft closeout stated one probe's `94 bytes` as though section length had one self-evident boundary | A second implementation measured 93 bytes because it excluded the separator LF before §2; the first included it. Both compared opening to disk equal and both negative controls diverged. The immutable claim held; the byte-count predicate was undeclared | A differently-shaped main-lane verifier, before the closeout committed | Every byte count names its delimiter rule. When two probes differ by one, reconcile the exact boundary rather than average the numbers or pick the expected one |
| ERR-2026-09-02-046 | HARNESS-MISUSE | the docs/19 widget rebuild ritual, first attempt (S114, Fable 5.1) | The chain's step [1] printed `RED: fmt` and I told Rab: *"`cargo fmt --check` went red at step 1 — the same gate that turned CI red in S101."* | `cargo fmt` never ran. My script did `cd windows-widget/` and cargo reported *could not find Cargo.toml* — the crate is `windows-widget/src-tauri/` (close.sh:30 already knew). The exit code was about launch mechanics, and I read it as the gate firing and reached for a historical incident to explain it | Reading the task's output file instead of trusting the summary line: the first line was cargo's usage text, not rustfmt's diff | A non-zero exit from a wrapper is a statement about the wrapper until the tool's own output has been read. Name the gate only after seeing the gate's output — and never attach a symptom id (SYM-020/S101) to a failure whose text you have not read |
| ERR-2026-09-02-047 | HARNESS-MISUSE | three read-only probes in one call: `find` for Cargo.toml, `sed` on docs/19, `grep` on close.sh (S114, Fable 5.1) | All three came back empty. For ~one turn the working picture was: no Cargo.toml under windows-widget, docs/19 absent, close.sh absent | All three exist and two had been read successfully earlier in the same session. The prior call had ended with `cd windows-widget && …`; the Bash tool persists cwd between calls, so every relative path resolved one directory too deep and simply missed | Self-caught before it reached Rab, by the project's own rule 4 (a failed probe is never a negative observation) — files I had already read cannot have become absent, so the probe, not the tree, was wrong | After any `cd` in a Bash call, the next call's relative paths are suspect: use absolute paths for every probe, or `cd` back to the repo root first. Emptiness from a relative-path probe is UNREAD until re-run absolute |
| ERR-2026-09-02-048 | PROBE-SHAPE | the staged widget exe for J20 (S114, Fable 5.1) | Told Rab twice, in direct answer to his question, that `1CF604CD` *does not exist anywhere on this machine* and *there is nothing to copy*; wrote the same into OPEN-TASKS J20 (`f41dd1e`) and the session record; then started a rebuild that overwrites that path | It existed at `windows-widget/src-tauri/target/release/`, the true staging path, mtime 08-31 18:11. Two probes failed differently and I promoted their agreement: (a) `ls` on `windows-widget/target/release/` -- the wrong directory; (b) `find /c/Users/Bndit -name ...` with `2>/dev/null` -- which, reproduced afterwards, lists the installed exe and NOT this one, while the same predicate scoped to `windows-widget/` lists it: the broad traversal silently truncates before `Projects/`, and suppressed stderr hid that it had. Both shared the assumption that an empty result meant absence (SYM-001's shape). CONSEQUENCE: the build I started on that premise overwrote the artifact before a preservation copy could be taken | The rebuild's own step [0] `pre-state` printed `artifact ALREADY present: 1CF604CD` -- a third probe with a different shape, at the path close.sh:30 names -- but it was read only after the build was already past step [5] | A negative from a broad, stderr-suppressed traversal is UNREAD, not absent. Scope the probe to the tree you mean and let stderr through. Before telling the principal X does not exist, read the path the project's own ritual says X lives at (docs/19:74, close.sh:30) -- it was on record. And before starting anything that overwrites a path, LOOK AT THE PATH FIRST (the system rule) -- step [0] existed for exactly this and I did not read it before launching |
| ERR-2026-09-02-049 | PROBE-SHAPE | the S114 standards fleet's corpus survey (Opus mapper), caught by the Fable verifier | mapping.md reported `/MarkInfo /Marked = False` on five files with 213+ StructElem each and built the claim 'Marked is not the gate' on four of them; design.md inherited it | Three of the four were a **pymupdf 1.28.0 parse bug**, not the files: `Document.markinfo` (`pymupdf/__init__.py:5855-5867`) splits the MarkInfo dict on `/`, does `key, value = v.split()` on the bare `Type` token when the dict is `<</Type/MarkInfo/Marked true>>`, catches the exception and returns all-False. `doc.xref_get_key(cat, 'MarkInfo')` on the same files returns the literal `Marked true`. One file (`ISO_TS_32003`) genuinely has `<</Marked false>>` with 393 StructElem, so the trap is real on 1 of 4, and the design's corollary stands on that one. A SIBLING TRAP in the same API surface: `xref_get_key` returns the TRUTHY tuple `('null','null')` for an absent key, so `if v:` reads absence as presence | The verifier read each file three ways (Document.markinfo · xref_get_key · raw catalog bytes) and the three disagreed on exactly the files with a `/Type` key ahead of `/Marked` | A convenience accessor is a PROXY for the object it summarises. Before a survey column becomes a claim, read the raw object for at least the rows that decide the claim — and any future `pdf_structure.py` must read MarkInfo via `xref_get_key` and test `v[0] != 'null'`, never `bool(v)` or `Document.markinfo` |
| ERR-2026-09-02-050 | PROBE-SHAPE | the S114 ledger diagnostic's commit attribution (Fable 5.1) | First run printed `by trailer: Codex 9 · Claude 14` for the 23 unledgered commits | Codex 3 · Fable 20. The script tested `"Codex" in <commit body>` — a substring over the WHOLE message — while my own messages name Codex in nearly every subject line (`CDX-0043`, `Codex closed F1`, `flagged to Codex's lane`). Six of my commits were attributed to the other vendor by their own prose. The label said `by trailer`; the code never looked at the trailer | Self-caught reading the per-commit list under the count: `f764042 CDX relay: Fable beat` cannot be Codex's. Re-measured on `^Co-Authored-By:` lines only. Never left the scratchpad; third PROBE-SHAPE row in one sitting (048, 049, 050) | A label is a claim about the method. If the printout says `by trailer`, the code matches the trailer line — `grep -E '^Co-Authored-By:'` — never the body. And an attribution count is read AGAINST its own list before it is quoted: one row that cannot be true falsifies the total |
| ERR-2026-09-03-051 | PROBE-SHAPE | Ticket C (sonnet builder) of the S114 signed-tickets fleet, reporting main.js line endings | 'main.js: LF before and after' tagged Observed | The file was MIXED (1115 CRLF + 24 bare-LF) before and UNIFORM CRLF after - 24 lines changed ending. The probe was `grep -cP '\r'` in Git Bash, which strips CR on read: it returns 0 on every file, so it cannot see the thing it claims to measure (SYM-029, the CRLF trap, filed 2026-08-1x and re-hit). The committed blob is unaffected (autocrlf normalises on add) | The Fable verifier counted bytes three ways (HEAD blob vs working copy, the index stat cache, a pre-edit tarball) and the report's claim did not survive any of them | A line-ending claim is measured by BYTES (`python newline=''` and count `\r\n`), never by a grep whose runtime strips the byte in question. Every builder brief already said so (rule 3); the rule needs to be a CHECK the verifier runs, which it now is |
| ERR-2026-09-04-052 | HARNESS-MISUSE | the S114 close's two-clock verification (Fable 5.1) | A one-line Bash chain did `cd <memory repo> && git commit && bash .claude/skills/muster/muster.sh | grep ...` and the grep printed nothing; for one turn the two-clock verdict was reported as 'printed nothing' rather than as a verdict | `muster.sh` was invoked by a RELATIVE path after a `cd` into a different repository, so bash could not find it; the error went to stderr, the grep saw an empty stream. The clocks were never checked. This is ERR-047's exact shape - and ERR-047's own rule ('after any cd, the next relative path is suspect: use absolute paths for every probe') was filed by me, in this session, twelve hours earlier | Self-caught on reading the empty output: a check that prints nothing has not run (muster rule 4). Re-run with the absolute path in the next call | A rule in the bin is not a rule until it is in the HABIT: every probe path in a chained command is absolute, full stop - and a chain that changes directory ends there; the next probe is a new command. Filed as a repeat so the class count is honest (HARNESS-MISUSE x3 this session) |
| ERR-2026-09-04-053 | HARNESS-MISUSE | the memory CI-wording fix at the S114 close (Fable 5.1) | Ran a `python -c` one-liner carrying a Windows path built with escaped backslashes to patch two memory files | The shell consumed the escapes and Python saw an unterminated string: SyntaxError before any write. ERR-009's exact shape ("heredocs and inline code eat backslashes - write scripts to a file and run the file"), a rule this session's first sitting already re-hit and re-stated. Nothing was written; the chained `muster.sh` never ran and its exit was the chain's, not a verdict | Self-caught on the traceback; rewritten as a file using forward-slash paths (io.open accepts them on Windows), which needs no escaping at all | The rule needs a mechanical form, not a memory: for this lane, NO `python -c` with a path in it, ever - a path goes in a file, and a path in a file is written with forward slashes. Third HARNESS-MISUSE repeat of a filed rule in one session (047->052, 009->053): the bin is recording the habit, not changing it, until the rule is a check |
| ERR-2026-09-04-054 | HARNESS-MISUSE | the docs/55 register-sweep commit (Fable 5.1) | Wrote `docs/55-register-sweep/README.md` from a `python -c` triple-quoted string inside a bash double-quoted command; the README text carried backticked words (`⟨status 2026-09-04: …⟩`, `VERIFIED.md`, `open-list-2026-09-04.md`, `fleet-result.json`, `wf_081e7e5e-3c5`) | Bash treated every backtick span as command substitution, ran them as commands (`command not found` ×5 on stderr), and substituted EMPTY strings — the README was committed and pushed in `615b0ea` with those words missing. ERR-009's exact shape, re-hit after ERR-053 in the same session, and this time the corrupted file reached origin | Self-caught on the stderr lines in the same tool result; the README rewritten with the Write tool and committed as a correction (the corrupted version stays in history) | The rule is now mechanical for this lane: NO Markdown or code text in a `python -c` or heredoc, ever — anything with a backtick, a backslash or a `$` is a FILE written with the Write tool. Fourth HARNESS-MISUSE repeat of a filed rule in one session (047→052, 009→053, 009→054): the pre-tool hook already warns on this shape and was ignored twice; the next repeat should block, not warn |
| ERR-2026-09-05-055 | RUNTIME-RESIDUE | the S115 scout pass (Fable 5.1) | Ran a read-only manifest probe as `uv run python -c ...` with the shell's cwd inside `windows-converter/` (a `cd` in an earlier compound command had persisted) | `windows-converter/` carries a `pyproject.toml`, so `uv run` resolved it as a project: it created `windows-converter/.venv/` (rapidfuzz + 1 package installed) and an untracked `windows-converter/uv.lock` in the repo working tree - residue from a probe that was supposed to read a JSON file | Self-caught on the tool output (`Creating virtual environment at: .venv` / `Installed 2 packages`) and on the next `git status` (`?? windows-converter/uv.lock`); both removed before any commit; nothing reached origin | Two rules, both mechanical for this lane: (1) never `cd` inside a compound Bash command - the cwd persists into later calls and every relative path after it points somewhere else (three later probes in the same sitting failed on `No such file or directory` for exactly this reason); (2) `uv run` only with `--no-project` (or from a directory with no pyproject), and `git status` after any tool that can write into the tree. A probe that can create files is not read-only, whatever it reads |
| ERR-2026-09-05-056 | CONTROL-LEAK (new class) | the S115 Atlas re-measurement fleet (Fable 5.1 coordinating) | Planted one false GROUND line as the fleet's decoy and, before the fleet returned, wrote that decoy's exact text into the session closeout (`sessions/S115-desktop-2026-09-05.md`, the 16:0xZ entry) as part of the work log | Both lanes read the closeout as ground and found the decoy NAMED there - the measurer's own report says so out loud. They also caught it by measurement (`git log -S` over the file's whole history), so the verdict stands, but the control no longer proves what it was planted to prove: a lane that had only read my notes would have scored the same catch | Self-caught on reading the fleet's report | A planted control lives ONLY in the coordinator's scratch until the fleet that carries it has returned; the closeout names decoys AFTER, never before. Same rule for the verify fleet's decoys, which this session kept out of the record until the verdict |
| ERR-2026-09-05-057 | PROBE-SHAPE | the same fleet's measurer lane, under a brief that said READ-ONLY | The lane ran `bash .claude/skills/muster/close.sh --help` to observe the DOCTOR and CENSUS rows, and its report described the result as 'a live row' from a help mode; the refuter lane then ran `close.sh 9c1446a` in the background and killed it | `close.sh` has no `--help`: `PIN="${1:-}"` swallowed the string as the pinned SHA and the whole close pipeline ran against a bogus ref (a close-adjacent script, whose real job includes the push gate). Nothing was committed or pushed - verified afterwards: origin unchanged at fbfa80a, reflog clean, the script contains no push/commit/add - but the brief's read-only law was a sentence, not a list, and a lane read a close script as a probe | The refuter named it; the coordinator verified the side effects | A lane brief ENUMERATES the scripts a lane may run; `close.sh`, `muster.sh` and `open.sh` are never a probe for any lane, and a script with no argument parsing is never given a flag to see what happens. Added to the S115 fleet-brief template |
| ERR-2026-09-05-058 | PROBE-SHAPE | the S115 merged-tree battery (Fable 5.1), then the verify fleet, then the push | Reported 'ruff linux-converter clean (hard gate)' as a CI-equivalent reading and pushed 13 commits on it | CI's 'Lint (linux-converter)' step runs BOTH `ruff check .` AND `ruff format --check .`; the battery ran only `ruff check`, so an unformatted `exporter.py` (the J33 lane's edits) passed every local gate and turned run 190 on 8b7afea RED. The brief told six refuters and a verifier 'ruff as CI runs it' and none of them read ci.yml's step either | Rab: 'ci failed' (his eyes, not a gate); the job log named the step and `Would reformat: converter/exporter.py` | A local gate that claims to be CI's runs CI's COMMAND, copied from ci.yml, not a remembered approximation of it; the S115 battery and every future fleet brief carry `ruff format --check .` beside `ruff check .` for linux-converter (and `cargo fmt --check` before clippy for the widget, which docs/19 already says). Fixed the same hour: format-only commit, CRLF kept, test set unchanged, CI re-observed |
| ERR-2026-09-05-059 | METER-CONFUSION | the S116 open (Fable 5.1) | Read run 192's jobs API and reported to myself "the three S108 warn-only steps all completed success" as the arming evidence for S108-9. The meter measured the neighbour: a `continue-on-error` step's API `conclusion` is the POST-override value, and the job LOG showed the windows-converter Lint step exited 1 with 50 errors. Caught by a local `ruff check` disagreeing (50 errors) BEFORE any edit, instruction or recorded claim rested on it; the log was then read and that step left warn-only (SYM-075). Rule: for a step that carries `continue-on-error`, the API conclusion is not a reading — the log (or the pre-override `outcome`) is |
| ERR-2026-09-05-060 | ORDERING | the S116 open (Fable 5.1) | Executed two of Rab's five signed slots — the ThinkPad pull+restart at 21:09Z and the real `--reaudit` at 21:17Z — BEFORE creating `sessions/S116-desktop-2026-09-05.md`; Phase 6's "recording precedes action" was violated for both. Each ran on `Observed` premises and Rab's own words and neither outcome was in doubt, which is exactly why the order slipped: cheap-looking acts feel like reads. The record was created at 21:2xZ with both acts admitted in its §4 (F0). Rule: the commission file is the FIRST commit of a session, before the first signed act, however cheap the act looks |
| ERR-2026-09-09-061 | DENOMINATOR | the S118 close (Fable 5.1), caught by Codex's independent verification (MSG-CDX-0047) | Wrote "the 472 passing chunks still lose 3.04 % of windows" in S118 §7/§8, in the relay and in memory. The 3.04 % was Σ(1−s)/472 — the UNWEIGHTED mean loss per passed chunk — not a share of windows: the window-weighted loss is 580 / 19,835 = 2.92 % and the whole-document loss 581 / 20,589 = 2.82 %. Each figure was true of its own population; the label named a different one (docs/34: numerator, denominator, conditions — the third was missing). Reached Rab and the bus. Rule: a "% of windows" must be windows over windows; a per-chunk mean says "per chunk"; when a lane's number and a document number sit in one paragraph, print both denominators |
| ERR-2026-09-09-062 | ORDERING | the S118 lever commit (Fable 5.1), caught by Codex (MSG-CDX-0047 correction 2) | Recorded the 0.50 → 0.80 lever as commit `c5709d7` in S118 §7, OPEN-TASKS J32 and the memory block, when the analyst.py edit had been `--amend`ed into `aa93818` (the J41-amendments commit) a step earlier; `c5709d7` carries docs and the register only. Runtime was unaffected (aa93818 precedes the re-run), the record was. Rule: after an amend, re-read `git show --stat` of BOTH commits before naming either in a record; better, never fold a signed lever into an unrelated commit — one signed change, one commit, its SHA read back |
| ERR-2026-09-09-063 | ORDERING | the S120 open (Fable 5.1), self-caught while writing §7 | "The record precedes the act" — S120's §1 was to be committed before anything was built (ERR-060's lesson, filed four days earlier) | `coordination/relay_numerations.py` was drafted at ~19:2xZ, before `sessions/S120-desktop-2026-09-09.md` existed; the commits are ordered record-first (`17f4509` then `280cfc7`) but the authoring was not — a measurement script is an act | own work log, 19:3xZ, while listing event times per Rab's addition | Nothing is written into the tree before §1 is committed — not a probe script, not an instrument; scratchpad probes are the only pre-record writes allowed, and an instrument is not a probe |
| ERR-2026-09-09-064 | OVER-CLAIM | the S120 fleet's L2 builder (Sonnet), caught by its refuter (re-run after the power loss) | Its negative-control row said disabling the envelope check flips "5 of 166" suite cases including the NR-07 meter case | Re-violating identically flips 4 of 166: the meter case exercises only the conforming path and cannot flip; the four substantive negatives fire as claimed | the refuter reproduced the exact violation and compared printed lines, not the summary | A negative control's evidence is the PRINTED line of the violated run, pasted, never a count remembered from a summary; the coordinator merged the code (guards fire under the refuter's hand) and filed the report's line here |
| ERR-2026-09-09-065 | PROBE-SHAPE | twice in one session: (1) the S120 L4 builder's watcher fixture; (2) the relay-watch Sonnet handler for MSG-CDX-0049 | (1) "21/21 — every signal driven"; (2) "reads only: inbox, status, check" | (1) the fixture's `append_header` stamped SECONDS, which `ENTRY_META_RE` rejects, so gate.py never parsed the fixture's hand-appended headers as entries and the ungated path (B4/B11) was unreachable — 25/25 green while the live watcher fired a phantom `INBOX MSG-CDX-0027`; (2) `check` WRITES the lane's state (it settled idle→idle, harmless) | (1) a probe script replicating the live case against a fixture with minute stamps; (2) the handler's own report listing `check` among its reads | A fixture that cannot reach the guard is a proxy (docs/47 law 4): before trusting a green, plant the live defect and watch the case go red; a brief's read/write list names every command that writes (the brief now says `check` is a write) |
| ERR-2026-09-09-066 | PROBE-SHAPE | the S120 post-close B13 tripwire drafts (Fable 5.1), caught by the suite itself | Two drafts of the two-writer tripwire were 'the guard' — one assembled entries in a canonical rebuilt shape (HEAD ≠ tree by blank lines after every commit), one restored a fixture by opening it for writing before reading it (an empty log for the next case) | The suite went red on both; a debug print (`text_len=0`) named the second | A tripwire is refuted by running it against the live shape before it is trusted; a file is read into a variable before it is opened for writing |
| ERR-2026-09-10-067 | STALE-GROUND | the S122 verification fleet's ground (Fable 5.1), caught by the V2 refuter and the V3 lane | The ground told every lane 'python = marker-env/Scripts/python.exe' — Fable's interpreter — for reproducing CODEX's suites, though Codex's `VERIFICATION.md:175` pins a different one; three Python rows read REFUTED/PARTIAL for the ground's reason before the lanes found the pin | The V2 refuter switched to the pinned interpreter and reproduced 115/115 and 86/86; V3 root-caused the launcher pid mismatch (now SYM-081) | When verifying another lane's claim, the GROUND carries THAT lane's pinned environment, quoted from its notes, never the verifier's own — the difference is itself the first finding |
| ERR-2026-09-10-068 | STALE-GROUND | the S123 reader fleet's brief (Fable 5.1); the destructive act by an in-place Sonnet reader at 02:40:41Z, found by close.sh's DIFF row reading `peer-owned 0` and confirmed by `git reflog` (`HEAD@{1}: reset: moving to feat/library-pipeline`) | "Read the shared ground first" — the S120 fleet ground, whose first instruction was `git reset --hard feat/library-pipeline` FOR A WORKTREE — handed to three readers told to work in the main checkout with no worktree | One reader ran the reset in the shared checkout; Codex's uncommitted bytes were destroyed: its MSG-CDX-0064 relay entry (header survives only in the watcher log), its 0064 `sent` record, its 01:30Z beat. Nothing of Fable's was uncommitted. The ground was written for one mode and carried unchanged into another — the same class as ERR-011/016/067, with a destructive effect this time | `close.sh` DIFF `dirty 0 · peer-owned 0` on a tree that had held the peer's bytes at the open; `git reflog`; the three agent transcripts all cite the line at 02:40:41–42Z | A ground names its MODE and its forbidden verbs; a lane that is not in a worktree may not run `git reset/checkout/clean/stash/restore`; the coordinator re-reads every instruction in a reused ground against the new mode before handing it over — the residue section is the least-verified part of a report and the FIRST COMMAND is the most dangerous |
| ERR-2026-08-21-069 | PROBE-SHAPE | S106, the P-1 caption triage's precision reported to Rab | "83 % precision" for the triage | The 83 % was measured on an ad-hoc classifier (an ILLUSTRATION-frame precedence) that the shipped code did not implement; the shipped rule was a different rule | Running the shipped code against the classifier instead of assuming they agreed (S106 §8 'Where I was wrong'); filed S124 (J55) | A number reported to Rab is measured on the SHIPPED artifact by the command that ships it — never on a neighbour of it (docs/34 conditions) |
| ERR-2026-08-21-070 | OVER-CLAIM | S106's appended correction, the SYM-050 repair formula | "Confirmed four independent ways" | The four checks were two evidence channels plus a range check, all parsing `_page_(N)_` and sharing one filename hypothesis — one check with a confidence interval (SYM-001), named in the same closeout's §5 and then not applied | The S106 Circle's correction, item 5; filed S124 (J55) | Two checks that share an assumption are one check; `Verified` needs a second method that can fail independently (docs/21 §1) |
| ERR-2026-08-24-071 | PROVENANCE-DROP | S109's fleet arc, D1 — two durable guard files (`muster.sh`, `open.sh`) | "THE DESCENDANT RULE (Rab's, signed 2026-08-24)", written into the guards by a lane | SIGNATURES.md had no such entry; my own brief had said "Rab has signed the fix" from a blanket authorization — an inference dressed as the principal's words and made durable (the fabrication direction of this class) | The S109 Circle (D1, HIGH); filed S124 (J55) | Only Rab proves Rab: a signature is quoted from SIGNATURES.md or the sign sheet, never inferred from an authorization; a brief that says "signed" names where |
| ERR-2026-08-26-072 | DENOMINATOR | S110 Fable lane — the analyst 'recovery' headline to Rab | "+8.70 points recovered — it worked" | 0.9452 (Marker→analyst, near-exact containment) minus 0.8582 (source→Marker, fuzzy window) subtracts two different denominators; 0.9452 was itself a fail on its own gate, and the analyst had deleted a 492-word run | J14/BRIEF §2.7's re-derivation (S110 §22-1); filed S124 (J55) | A difference of two ratios needs the same denominator and the same conditions on both sides (docs/34); otherwise it is not a difference |
| ERR-2026-08-26-073 | REGISTER-MISS | S110 Fable lane — six artifacts' authorship stamps | Bare "⟨claimed: Fable⟩" was the current form | Rab had corrected the form on 2026-08-24 in `coordination/authorship.md` §Correction — the file governing the act; the rule sat in the rulebook the act was under | S110 §22-2; filed S124 (J55) | Before an authorship/format act, read the file that governs it; a rulebook you did not open is a register you missed |
| ERR-2026-08-26-074 | PROBE-SHAPE | S110 Fable lane — the loss-sweep audit | "B: 8 LOST" | The audit counted a log verb (`SAW`) instead of disk state; the mechanism's bytes sat on disk, complete | The second pass, re-probing disk (S110 §22-4); filed S124 (J55) | Measure the object (the bytes on disk), never a neighbour that talks about it (a log line); a count taken from a log verb is `Inferred` |
| ERR-2026-08-28-075 | HARNESS-MISUSE | S112 Fable lane — a relay append chain | The append had run (the chain continued green) | A `grep -c` exiting 1 on zero matches short-circuited the chain, so the append never ran; "done" was an exit code, not a measured line count | Re-measuring the line count (S112 §10-4); filed S124 (J55) | A "done" for a write is the count of bytes after it, not the exit code of the command before it (SYM-077's kin: grep's exit status is not a verdict) |
| ERR-2026-08-28-076 | HARNESS-MISUSE | S112 Fable lane — MSG-FAB-0046 | One append | A shell quirk ran the append twice — MSG-FAB-0046 duplicated byte-identically; no idempotency on the append path | Byte comparison after the fact, then deduplicated (S112 §10-4); filed S124 (J55) | An append onto a shared log is checked by re-reading the tail (the id appears once), and the writer is idempotent (gate.py's later intent journal) |
| ERR-2026-08-28-077 | OVER-CLAIM | S112 Fable lane — the Okular digest's first framing | Full audit coverage (404/404 MATCH) | The popplerweb lane was unaudited; the coverage claim was blind to its own gap | The critic pass, before commit (S112 §10-4) — corrected into A49's caveat; filed S124 (J55) | A coverage claim names its denominator (which lanes) before its numerator; a lane not audited is UNREAD, never folded into a MATCH count |
| ERR-2026-09-10-078 | ORDERING | S126, J58 — the exporter change, its tests, the push, the ThinkPad pull and the unit restart (06:04:31Z) | The session was proceeding in order | The S126 record did not exist yet: J58's code, deploy and restart all preceded it (the S125 record had been written first; this one was not) — ERR-063's class, its second instance in six sessions | Noticed while listing what the record needed to say; named in the record's §7 and here; filed S126 | The record is the FIRST commit of a session, before any instrument or act (docs/28; muster Phase 6) — a card in a file is not a record; `open.sh` should refuse to print a clean card while `sessions/S<N>-*.md` is absent AND a working-tree change exists (a candidate tripwire, not built) |
| ERR-2026-09-10-079 | PROBE-SHAPE | S126, the J58 exporter change — CI run 235 on `3c39d3b`, red at `Lint (linux-converter)` (Rab: "Run failed") | "ruff clean" — I had run `ruff check` on the three files | CI's step runs BOTH `ruff check .` AND `ruff format --check .`; the second failed on my handler's formatting. Half of a two-part gate was measured and reported as the gate | Rab saw the red before I did; `ci.yml` read, the two commands re-run locally, `ruff format` applied (`119db7d`, formatting only), CI observed after; filed S126 | Before claiming a gate is clean, run the gate's OWN command as CI spells it (`ci.yml`), not a neighbour of it; a lint claim names both halves (check + format) |
| ERR-2026-09-10-080 | ORDERING | S127 — the guard v4.5 fix and the card's auto-logon row (`cb964a6`, `ca3768a`) | The session was proceeding in order | The S127 record did not exist: both instruments preceded it — ERR-063's class, the THIRD instance in three sessions (S120, S126, S127); a discipline that fails three sessions running is not a discipline | Noticed while writing the record's log; named there and here; filed S127 | Mechanized the same session (J63): a hook refuses a tracked-file write and a commit while the session's record is absent — the rule moved from memory into the harness, where the last three failures say it had to be |
| ERR-2026-09-10-081 | OVER-CLAIM | S128–S129 — the reading-bench `SKILL.md`'s first draft, the S129 record's §7 (`8effd42`) and the reply that carried it | The DDIA manifest flags 139 pages | It flags **144** (`len(pages_flagged)` over 662 scored, re-read from the manifest at S129's second pass); the 139 was written from the memory of an earlier read and never re-measured at the moment of writing — rule 3 of the tag law (a number is re-measured, never quoted), inside a file whose first law is "real bytes only" | The rebuilt page's dial read 144 against the prose's 139 — the instrument disagreed with its own documentation; counted with a script the same minute | SKILL.md corrected with the correction named in place; §7's line left standing and pointed at by §8 F2 (appends never erase). The rule it sharpens: a number that goes into prose is produced by a probe run in that sitting, and the probe is named beside it |
| ERR-2026-09-11-082 | RUNTIME-RESIDUE | the night of 2026-09-10/11 (recorded S130) — a PowerShell probe while measuring the process feed | I was reading a script's text | The probe dot-sourced a `.py` path (`. "…\count_flagged.py"`): PowerShell handed the extension to its file association and VS Code launched, spawning two Claude SDK instances that then sat in the very feed I was measuring — an inert read reached a wider scope, and the instrument recorded its operator | The refutation fleet's critic found the launch in the raw feed at the probe's second, parented to my shell | A probe never invokes a path it means to read: `Get-Content` by literal path, never `.` or `&`. Filed with 083, which is what I then said about it |
| ERR-2026-09-11-083 | OVER-CLAIM | the same night — the reply that walked Rab through the feed | VS Code's appearance in the feed was Rab's own launch | It was mine (082): I named him as the actor of a process I had spawned, from the plausibility of the picture, not from the parent pid | The critic lane, against the raw log (the parent chain led to my shell); corrected to Rab in the next reply | An actor is read from the parent chain, never from who "would" have done it — and a claim about Rab's hands is consequential: `Inferred` said out loud, or not said |
| ERR-2026-09-11-084 | OVER-CLAIM | the phone hook build, 2026-09-11 — the display-scale script returning error 31 (ERROR_GEN_FAILURE) on every DisplayConfig call | "The harness blocks the DisplayConfig API" — carried to Rab as the cause, by me and by both fleet lanes | The cause was in the script: Windows PowerShell copies a nested struct field on read, so `$req.header.type = -3` wrote into a throwaway copy and the request went out with a zeroed header; the API refused a malformed packet. Fill the header, then assign it whole — the selftest read 5/5 from Rab's console at once | Rab's console ran the same script outside the harness and failed the same way, which removed the harness from the picture; the struct copy was then found by probing the request as marshalled | Three lanes agreeing on an unprobed cause are one inference. The harness is blamed only after the same command fails outside it — a negative control that costs one run — and the correction goes to Rab in the reply, named |
| ERR-2026-09-11-085 | PROBE-SHAPE | S130's ledger row (`34e577c`, 17:2xZ) — its shape was taken from the first 900 characters of the S129 and S128 rows (`cut -c1-900`) | The row's contract is "date · machine · text, with the closing SHA somewhere in it" | The table has FIVE cells and `muster.sh` [3b] reads the closing SHA from the LAST non-empty cell as a bare 7–40-hex token; my row had three cells with the SHA inside prose, so the next open's clock check read `row139(no-sha)`, selected S129, and declared an INCIDENT — thirty minutes after a clean close | The mock muster Rab asked for ("what would happen next session"): `muster.sh` said it in one line. Rule 2 of the tag law: sampling never promotes — a row's shape read from a truncated line is `Inferred`, and the reader that would consume the row was on disk and costs two seconds | A ledger row is validated by ITS READER before the push (`muster.sh` after the row commit — J69 puts it in the close); and read the WHOLE line of the thing you imitate, never `cut -c1-N` of it |
| ERR-2026-09-12-086 | OVER-CLAIM | S130 (2026-09-11) — the private repo's README, its first commit's message, the record's §15 and the memory file `file-portal-private-workshop` | "The Desk's log `relay-desk/data/desk.jsonl` is tracked" — written four times | `relay-desk/.gitignore` line 1, `data/`, ignored the whole directory; git had never seen the file. The tree staying CLEAN after Rab's post landed was the tell | Rab's sign-off post at 18:32Z produced no diff; `git check-ignore -v` named the rule | A file called tracked is shown tracked — `git ls-files <path>` prints it — before the word is written; fixed in private `77dba4a` (`data/*` + `!data/desk.jsonl`, 13 lines tracked from there) |
| ERR-2026-09-12-087 | HARNESS-MISUSE | S130 post-close and S131's open (2026-09-11/12) — four probes in one sitting | A one-line probe is faster than a script file | The guard denied all four, each a shape ALREADY on the register: an inline `python -c` with braces (J64's sixth shape) twice, a `$FP` head and a PowerShell `$_.CommandLine` head (the designed deny) — ERR-009's rule ("scripts in files") and `pace-and-known-errors` ("never repeat an error already in ERROR-BIN") both ignored in the moment, four times, by the lane that filed them | Each deny read back in the same call; the tenth shape (`find -printf`) was the only NEW one | In the shared checkout a probe is a FILE run by literal absolute path, first time, every time — the guard's deny is the cost of the shortcut, paid in a round trip each; the list of known shapes lives in SYM-110's first cell, read it before typing a one-liner |
| ERR-2026-09-12-088 | BOUNDARY-OMISSION | S133's close chain (`close_chain_generic.sh`, 08:4xZ) | The chain stops on a measured red | It stopped on THREE reds I had named (uncommitted work, glass, rust) and not on `close.sh`'s exit code: a REGISTER red (the register untouched, no dated line) went through to the row and the push. And the gate CI runs on every push — `schema_registry_selftest.py`, the A4 hard gate — was never run locally: the `audit/reaudit` event gained `repairs_source` / `repairs_entries` in `34a46db`, the checked-in `schemas.json` did not, and CI went RED on the row commit `f8770cc` (run 252, python job, step 16). S113's rule — a build's new keys need same-commit dispositions — was in memory and not in the chain | `close.sh` printed `close exit 1` in the chain's own output; `ci_observe.py` printed the red 90 s after the push | A chain gates on the EXIT CODE of the gate it wraps, never on a list of reds it remembered; and before any push the chain runs the gates CI will run (`schema_registry.py --check`, `acceptance.py`) — one second each. Both wired into the chain in the same commit as this row |
| ERR-2026-09-12-089 | ORDERING | S137's J64 patch of the live git guard (15:3xZ) | The suite runs green before the next shell call the guard will read; a broken guard is repaired by a tool the guard does not read | The patch's heredoc fold called `guarded`, a closure inside `decide()`, from module level; the suite said J64/11 RED with *internal error (name 'guarded' is not defined) — fails closed*; I read the colour, went to the specimens for the reason, and then sent the REPAIR through a Bash heredoc — the exact shape the broken fold now denied on every call. One call lost; nothing damaged (fail-closed did its job) | The guard's own deny text on my repair call | When the live guard is under edit: repair through `Edit`/`Write` (the guard reads the shell, not the editor), run the suite to green, THEN the next shell call. A red case's REASON is read before the next act, not its colour |
| ERR-2026-09-12-090 | HARNESS-MISUSE | S137, a lint baseline for `guard_git.py` (15:3xZ) | A comparison against HEAD never asks the shared checkout for a working-tree verb | I ran `git stash -q && ruff … && git stash pop` in the main checkout to lint the committed version; the guard refused `git stash` as tier 1 (J56, ERR-068's law). ERR-068's family, repeated — caught this time by the guard built for it | The guard's deny line | The baseline is `git show HEAD:<path> > <scratch>` and lint the copy; the shared checkout is never stashed, reset, or checked out by a session |
| ERR-2026-09-12-091 | STALE-GROUND | S137 §18, the S137 memory block, the S137 Desk plan ("the 19 briefs") | A count is re-measured, never quoted (tag law rule 3) | "19 briefs" was S134's count, carried through S135–S137 into a record section, a memory block and a plan handed to Rab while two briefs had been added at S136; the S138 open's `ls agent-scripts/*/*.brief.md \| wc -l` said 21. The planted control brief carried the same typed 19 and lane C measured it against the library | The S138 open (I re-measured before briefing the lanes); lane C's count | A number that names the library is a command's output pasted beside the number, in the record and in memory alike; `brief_check.py` prints `briefs: N` at every close from now on |
| ERR-2026-09-12-092 | BOUNDARY-OMISSION | S138's close chain (`close_chain.sh`, 16:22Z) | A chain's exit code carries the last gate it ran (ERR-088's own law) | The chain observed CI on the row commit `e3021fb`, printed `conclusion=failure` and `step 15 failure Governance acceptance`, and then printed `chain exit 0` — the observe step's exit code was piped into `grep \| head` and never propagated. ERR-088 had wired the gates BEFORE the push; the reading AFTER the push was left unwired. And the local gate had passed at the CONTENT commit two commits before the pushed HEAD | The chain's own last lines, read by me | The chain exits 10 on a red row commit and names the log reader (`ci_step_log.py`, new); the brief says the gates run at the content commit and the pushed HEAD is two commits later |
| ERR-2026-09-12-093 | ORDERING | S139, the foolproof lanes (16:5x–17:3xZ) | A lane reads a fixed target; its control judgement is about the code it was given | I fixed the smoke's four findings in the very files four lanes were reading (Rab's "fixed immediately" and the lanes' 36 minutes overlapped); two lanes reported the moving target — one named an already-fixed script as the planted control, one named my in-flight diff — and one lane re-read a file mid-review and found it changed. The fixes were right; the order was not | The lanes' own control sections | Lanes first, fixes after (or the fixes on their own branch); `SEQUENCES.md` law 5 |
| ERR-2026-09-12-094 | HARNESS-MISUSE | S140, stopping a stale background check (18:3xZ) | A kill by command-line pattern excludes the killer | `Get-CimInstance Win32_Process \| Where-Object CommandLine -like '*promotion_check.py*' \| Stop-Process` matched the PowerShell that was running it (its own command line carried the pattern) and a harness process besides the stale python — the tool call died with exit 255. Nothing of Rab's was touched | The exit code and the process list the call printed before dying | Match the target's executable and script name, exclude `$PID` and the harness's shells; prefer the tracked task's own stop (TaskStop) or a STOP file the process watches |
| ERR-2026-09-12-095 | PROVENANCE-DROP | S140, docs/15 §12.2 as first written (18:4xZ) | A number names the script that produced it | `STRICT (markup + hyphen only): 1.0 / 0` sat inside the sentence 'Measured at promotion (promotion_check.py …)' — that script never computes STRICT; the number came from `promotion_variants.py`, run minutes apart. Two lanes and the refuter caught it | The Readability and Test lanes read both scripts and the output file | Every measured number in a doc names the script AND the run that printed it; a paragraph that cites one script carries only that script's numbers |
| ERR-2026-09-12-096 | OVER-CLAIM | S140, docs/15 §12.2 and the module docstring as first written | A classification in a signed doc is the code's, run | I wrote that *Zero to One*'s `ESCAPPING` 'was a case/punctuation-shaped edit' — the module's own selftest case, written the same hour, names it a substitution + deletion; I never ran the pair before writing the sentence | The Readability lane ran `reconcile("E SCAPING the room", "ESCAPPING the room")` | A class name in a doc is pasted from the log line that produced it, never reasoned |
| ERR-2026-09-12-097 | RUNTIME-RESIDUE | S141 E5, `guard_record.py` learning to log its denies (23:54Z) | The guard's selftest is inert — it runs the hook against a throwaway root | The hook now writes every deny to `coordination/private/git-guard.log`, and the selftest's eleven fixture denies went into the PRODUCTION log (five lines, `guard=record`, the temp root's paths) before the selftest redirected `FP_GIT_GUARD_LOG` — the very count the cross-check ticket exists to make trustworthy | `grep -c guard=record` on the log right after the first green selftest: 5 | A selftest of anything that WRITES sets the write target to a throwaway first (the guard_git selftest already did; this one was copied without it); the five lines were removed by a scratch script with a copy of the log kept, and this row is the record of the removal (the log is append-only for DENIES; a fixture line is not a deny) |
| ERR-2026-09-13-098 | QUOTING | S141 E6, patching `guard_git.py` (00:1xZ) | A `python -c "…"` one-liner edits a file | The double-quoted string carried a literal `` `git push` `` (copied from the deny message it was editing); bash ran it by command substitution BEFORE python saw the text — `git push` executed twice (both `Everything up-to-date`, nothing moved), and the guard did not read a wrapper's quoted string as a command | The two `Everything up-to-date` lines on the terminal where the patch's own output should have been | A patch is a FILE run by path (SEQUENCES §0 law 1; ERR-009's remedy) — never a `python -c` with quotes carrying text that mentions a command; a backtick inside double quotes is a command to bash, whatever it is to python |
| ERR-2026-09-13-099 | RUNTIME-RESIDUE | S141 E7, `analyst.py` learning to ledger its chunk journal before the rmtree (00:26Z) | The analyst's selftest is inert — scripted generations, a temp work dir | Its FIRST run after the change called `dumps/dump.sh` in COPY mode 36 times (one per `process()` in the suite, several of them direct calls that bypass the `run()` helper) — 36 rows D0006–D0041 in the PUBLIC tracked ledger, 36 fixture journals under `dumps/evidence/`, before the `FP_DUMP_LEDGER` throwaway existed; the real orphan I then ledgered became D0042 | `git status dumps/` after the first green run; the next dump's id read D0042 | The same rule as ERR-097, two hours older: a writer's selftest sets the write target to a throwaway BEFORE the first run, at IMPORT, not inside one helper — and a new writer to a tracked ledger is run once against a throwaway by hand before its suite. The 36 rows, twins and files were removed by a scratch script with copies kept; D0042 keeps its id and this row explains the gap D0006–D0041 |
| ERR-2026-09-13-100 | DELEGATED-TRUST | S141, the ticket `unread-surfaces/watcher-stuck-drop-file` (written 09-12 ~21Z, built 09-13 00:5xZ) | The ticket's *born of* said the traceback was the full-width `：` in the name and 'the SOURCE FILE is still in `drop/` today, unmoved' - tagged **Observed** with `ls drop/` | `drop/` held no PDF at all (only `done/`, `failed/`, `.supersede`; `.intake-state.json` waiting 0; the folder's mtime 04:13Z that morning = the last move, hours before the ticket); the file sits in BOTH `done/` and `failed/` (two downloads, 07-22 21:12 and 21:20); the cause was another hand removing the source mid-conversion (SYM-124) | Reading the folder before writing the move: one `os.listdir` and the dir mtimes | A lane's reading relayed into a ticket is **Reported** until re-measured by me; the second reading 'confirmed' it by re-reading the same log, not the folder (SYM-001's one-check-twice). The tag law's rule 2: a ticket's *born of* cites the command AND its output, and I run it again before the build - the ticket template gets a `re-measured at build` line |
| ERR-2026-09-13-101 | BOUNDARY-OMISSION | S141 E7 partial closeout (01:0xZ), the push of `670b51c` | The episode's tickets were closed on their own selftests (deferral gate 19/19, muster 101/101, families 13/13) and pushed as done | The public tree carried a new event (`intake/move_failed`) with a stale `observability/schemas.json`; CI's A4 hard gate went red on the close commit — the one gate the S141 full close would have run (`schema_registry.py --check`, 2 s) and a partial close did not | `ci_observe.py` after the push: `conclusion=failure`, then `ci_step_log.py`: `FAIL checked-in schemas.json is current` | A partial closeout is a close: before its push it runs the same-commit gates the full chain runs on the public tree — `schema_registry.py --check`, `acceptance.py`, `glass_detector.py --since <pin> --enforce`, `error_families.py --census` — as one script (`partial_close.sh`, E11's contracts pass writes it into SEQUENCES §3); a red found by CI after the push is a red I could have measured before it |
| ERR-2026-09-13-102 | STALE-GROUND | S144 E3a, the launch of the four re-conversions on Rab's word (09:12Z) | that a re-converted book whose audit fails would HOLD with a sidecar, as the S142 triage wrote and as the 08-25 and 09-12 holds had done | the converter ships every verdict unless `audit-mode.txt` reads `enforce`; it had read `report` since 09-12 02:59:52Z (no record of the writer); Ashby (verdict fail, degeneration) shipped, and the exporter's first-ingest path — no verdict guard of its own — put it in the vault (54b471d7, 11:31:10Z). AND (C-024, S145): the open card had printed `levers audit=report` at the S142, S143 and S144 opens — the lever was in front of me three sittings running and unread | the ThinkPad's `EXPORTED` line read at 11:31Z against the manifest's `verdict fail`; Beer's run killed by hand two minutes in, the queue stopped, the three decisions put to Rab | a launch that can ship reads the lever that decides shipping FIRST (`audit-mode.txt` beside `.gpu-lock`, `nvidia-smi`, `query user`); "fails hold" is a lever's state, not a law; the open card prints the mode (SYM-131) |
| ERR-2026-09-13-103 | OVER-CLAIM | S144 E4's record of the 20-page timing probe (09:09Z) and E5's repeat of it | that the sidecar's `save_output` failed because its output folder did not pre-exist ("the converter pre-creates it, the probe did not") — a cause, stated as fact | marker creates the folder itself (`os.makedirs(exist_ok=True)` in `get_output_folder`); the error was Windows' 260-character path limit on `<scratchpad>/…/<71-char stem>/<71-char stem>.md` (~300 chars), which `open()` reports with the errno of a missing file | the direct run's first range died identically WITH the folder pre-created (11:46Z); the path counted; a short root fixed it | a cause is a claim: name it Inferred until a probe that isolates it has run (here: `ls` the folder, count the path); an errno is a symptom family, not a cause (ENOENT covers a long path on Windows) |
| ERR-2026-09-13-104 | DELEGATED-TRUST | S145 E4's record ("The reading") and its Desk close (f433f13a, 16:18Z), OPEN-TASKS' struck `measurement/slice-200-direct` row, the S145 notes' finding 9 | that "the tracker's receipt for the window shows no stream, capture or game process (no Sunshine, no Moonlight, no OBS)" — a co-tenant of the card cleared by an instrument's list | the receipt lists process BIRTHS under a poll grid and holds no row for the 4,003 s converter pass born inside its window, nor for its sidecar (`page_range 0-199`: 0 rows) — a list that misses the longest process in its window is not a census; the co-tenant reading is UNREAD, not `none` (S4 FAILED-PROBE-RENDERED-AS-NEGATIVE) | S145 E8: the converter's own run reproduced the direct run to three seconds (1,698.6 vs 1,701 s), which moved the cause to the card's other tenant, and the receipt was re-read for it — the pass itself was absent (IB-018, C-025) | an instrument's negative ("none seen") is admissible only after its coverage is checked against something known to have been there — the pass was; a receipt's own header ("a process shorter than the grid is absent") is a coverage claim to test, not to trust |
| ERR-2026-09-13-105 | ORDERING | S146's open (17:33Z): the library smoke launched in the BACKGROUND and, while it ran, the episode-close script run in the foreground (receipts + ledger rows written in the private tree) | that the smoke could run beside other work — the act assumed it, no claim was made | the smoke's own write tripwire fired (`private unchanged by the smoke: CHANGED — a smoke run wrote something`, 43/44 RED): the smoke asserts all three trees unchanged for its whole run, so anything writing beside it is indistinguishable from the smoke writing — S142's lesson "the smoke runs ALONE" (in the memory index since S142) repeated; re-run ALONE in the foreground: 44/44 green in 75 s | the smoke's tripwire, read before any claim | a check that asserts "nothing changed" over an interval owns the interval: nothing else runs in it — foreground, alone, and the next act waits for its last line |
| ERR-2026-09-15-106 | BOUNDARY-OMISSION | `windows-converter/analyst_selftest.py` since S156 E1 (2026-09-15 05:2xZ): the S156 (a) case's negative control (`process()` without a reading) called with the REAL `_generate`, outside the try/finally that scripts it; every run of the file since (Ollama's log on 2026-09-15: eleven real generates at the selftest's run minutes, five of them cold loads — S156 ×6, S157 E1, E3, E6 ×2, E5's fix; the three of E6/E5 matched to the second, the rest by the minute, Inferred) made one real `/api/generate` with `keep_alive 30m` — qwen3:8b LOADED on Rab's card (6.3 GB) for thirty minutes at a time; the S156 journal entry told him "nothing on the card" | that the analyst selftest never touches a real Ollama (its own line 53: `analyst.unload = lambda: None  # never touch a real ollama server`) and that the sitting's cost to him on the card was nothing | one case did; the comment was a promise nobody enforced; `process()` swallows a backend error as a failed chunk, so even a blocked network would have passed 43/43 silently | the fleet PROCESS census built for J3 (S157 E9), run over the sitting's own window: `llama-server.exe` created 06:54:23Z still alive, matched to Ollama's log (three POSTs at the three selftest runs) and to the tracker (the selftest's pid 22 s before the load); a urlopen spy on a run named the case and the line | a boundary a test's comment promises is a boundary the test must COUNT: every urlopen the file reaches is counted and refused, and a final case reds on a non-zero count (the guard went red on the leak before the fix — its negative control — and green after); "nothing on the card" is a claim about Ollama's log and `nvidia-smi`, never about intent |
| ERR-2026-09-15-107 | PHANTOM-MONITOR | S154 E7's seat watcher (a Bash monitor polling `seat_free.py --min-idle 10` every 60 s, started ~2026-09-14 17:3xZ to launch the p.175 Marker probe when the card fell idle) — left ARMED after S155 ran that probe on Rab's word; found alive at S157 E10 (07:3xZ) by the tracker's receipt (54 runs in the sitting's window; its output `still held (840 min)`) — fourteen hours of polling `query user` and `nvidia-smi` on his machine for a purpose that had ended, and a `SEAT FREE` line that would have arrived as a notification with its purpose gone | nothing was claimed — the watcher was forgotten across two closes and a compaction; the carry-over named the Desk monitor and the tracker, not this one | the inverse of the class: not a one-shot remembered as a watcher but a watcher forgotten as a one-shot — alive, polling, with no reader | the fleet process census (E9) listing `python.exe seat_free.py` fifty-four times in the window, then the tasks directory's output files | a monitor is a process with a purpose and an end: the carry-over and every close list the monitors armed (task id, purpose, the event that ends it), and the census at each closeout reads the list against the receipt — a poller with no listed purpose is stopped, not kept |
| ERR-2026-09-15-108 | DENOMINATOR | the lever `ANALYST_TABLES = True` (S154, on "all signed") over the whole shelf, on the strength of S151 E1's "zero wrong labels"; then S157 E5's shelf probe, which printed `labels 4` for Ashby beside the trims and splits the lane was reading — the column went unread for ninety minutes | that the rail route places no wrong label (S151) and that E5's shelf run showed "every ordinary proposal's fate identical" and nothing worse | "zero wrong labels" had Valentine's seven rails as its denominator; the lever's denominator is every table of every book; on Ashby the same route blanked four tables' row labels and wrote ELASTIC, IDEAS, ADDS, ABC over them — the E5 control (old layer vs new) compared two layers both wrong there, and "identical" was true and said nothing | reading the probe's `labels` column at E15, then the four tables' before and after (`| ELASTIC | <br> | | |` over the blanked `3 4 5 6`) | a number carries its denominator with it (docs/34): a route measured on one book is UNKNOWN on the next until the shelf is run and EVERY column of the probe is read — and a lever that goes ON for every book gets the shelf run before the switch, not after |
| ERR-2026-09-15-109 | EDIT-UNDER-EXECUTION | `sittings/S157/e24_measure_anchors.sh` (private), running in the background over fourteen anchor bundles (S157 E24, 09:15Z): the lane edited line 13 of the script (a hash suffix for the output slug, after the E5 lesson bit again — the Valentine and DDIA pairs had collided on a 60-char cut) WHILE bash was executing it; bash reads a script incrementally by byte offset, the edit lengthened the file by ~90 bytes, and the next line bash read was the Investment Valuation (4e, 2025) line AGAIN — the run's log shows that book measured twice (54 s, then 53 s) and its first report truncated by the second run's redirect | that a running shell script is a fixed program once launched, the way a Python file is (Python compiles the whole file before running; bash does not) | bash re-reads the script file as it goes; an edit that shifts bytes above the current line makes it re-execute or skip lines; the only reason nothing broke is that the tool is report-only and the second run overwrote the first with the same bytes | the run log (`e24/anchors_run.log`) carrying the same book twice, and two `coverage_rescore.py` processes for it seen by `wmic` at 09:16:43 (one the venv trampoline, one its child) | the sitting's own law, "never edit files a running lane is reading", applies to a SHELL SCRIPT the lane itself is running — copy it to a new name and edit the copy, or wait for the exit; a script the lane wants to edit mid-run should have been a Python file |
...
### 18. Current 43-row reading after ERR-039–043

`Observed 2026-08-28` from this register: **23 classes across 43 rows**. No class is new.
Row 039 was caught only after Rab challenged the apparent stall, so the “another mind” numerator
moves from **22 of 38** to **23 of 43**. Rows 040–043 all failed closed or were corrected before a
durable claim. `ERROR-BIN.md` remains named in **zero of forty-three** “How it was caught” cells;
this append changes the denominator, not the register's demonstrated prevention rate.

### 19. Current 45-row reading after ERR-044–045

`Observed 2026-08-28` from this register: **23 classes across 45 rows**. Both rows recur in
existing classes and were caught before a durable claim. The “another mind” numerator therefore
remains **23**, now **23 of 45**. ERR-044 is direct evidence that writing ERR-040 minutes earlier
did not alter the next harness; `ERROR-BIN.md` remains named in **zero of forty-five** catch cells.
