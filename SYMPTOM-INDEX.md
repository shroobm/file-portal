...
| SYM-018 | You push code and **no CI status ever appears** on the commit; lint/test regressions reach the branch unnoticed | The workflow triggered only on push to `master` plus `pull_request`; all work is direct-pushed to `feat/library-pipeline` and the project uses no PRs, so CI never fired — 203 consecutive commits shipped unverified. It also covered only 2 of 5 Python areas, excluding `linux-converter`, which writes the vault | `Observed 2026-08-09` (S67) | `fixed` — trigger now includes the feature branch; converter lint+tests added. **`cargo test` still absent from CI** | `.github/workflows/ci.yml` (trigger note in the header) |
