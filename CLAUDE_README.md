...
| 2026-08-16 | Desktop | S82: **raising `n` exposed the method, not the result.** At n=30 the gap WIDENED (0.77× / 0.62×) — an artefact of **arm order**: decode rate tracks POSITION in the run (r=−0.78) not output length. Same config, same chunks: 97.7 tok/s in a 7-min run, 77.7 in a 25-min one. **0.95×, 0.81×, 0.77×, 0.62× all WITHDRAWN.** Built the **A-B-A control** and watched it withhold (+18.1 % drift). Mechanism **Unknown** — not thermal. The token gate survives. SYM-035. Closeout: `sessions/S82-desktop-2026-08-15.md`. | 8fb374e |
