...
- ▶ Next up (user-gated): **the Assay build session** (docs/15 §13 — Rust `assay_status`/`audit_mode_get|set`, the `◎` line station + evidence card + `report ⇄ enforce` toggle, `_enforce_hold` wiring into ship/defer/resume, then the Beer flag→re-convert→re-audit→supersede remedy loop; needs the widget rebuild ritual); ThinkPad exporter **supersede flow** (unblocks the remedy's auto-swap — THE SUPERSEDE GAP) + enrichment wiring. Phase 5 Forgejo: still deferred, maybe never. Carry-forward: `min_chars_per_page=100` provisional — revisit after ~30 real conversions; analyst E2E-to-vault leg awaits the next genuinely-new document (dedup skips analyzed re-ships of already-vaulted sources by design).
