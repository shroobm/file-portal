...
**S32 — post-S31 hotfixes + widget rebuild (Desktop, 2026-07-21).** During S31 live-testing (Rab drove the install + drops), four real defects surfaced and were fixed + verified; this session commits them and rebuilds. All found by actually running the pipeline end to end.

1. **Auto-start crash (HIGH):** a Start-menu (GUI) launch has no console, so the widget's spawned Python watcher inherited dead std handles and died on startup before logging — "auto-start runs but nothing converts." Fix: `watcher.rs` + `preflight.rs` spawn with explicit `Stdio::null()`. (Masked before because dev/terminal launches had a console to inherit.)
2. **UI freeze (MED):** `vault_check`/`vault_pull` run `git fetch` over tailscale ssh with no timeout, on the main thread (Tauri runs sync commands there) → the whole widget went "not responding" every poll while the ThinkPad was offline. Fix: `main.rs` — make them `async` + `spawn_blocking`.
3. **Clean-lane VRAM thrash (MED):** the clean lane ran Marker uncapped, so a figure-dense born-digital PDF (Cybernetics models book, 91 pp) ballooned to the 10 GB ceiling and timed out at 60 min. The scan lane's cap is why Beer finished 439 pp. Fix: `convert_and_ship.route()` caps the clean lane too (`--recognition_batch_size 32`). Live-verified: re-convert peaked ~8.0 GB, no thrash.
4. **Audit false positives (MED):** the degeneration tripwire false-fired on the Cybernetics book — its dense tables tripped the zlib half, its repeated section headings tripped the repeated-line check. Fix: `fidelity_audit.degeneration()` — `OR`→`AND` (a loop is BOTH crushed-zlib AND high-trigram; tables have low trigram) + the repeated-line check now measures the longest CONTIGUOUS run (loops repeat contiguously; headings/tables are distributed). Re-verified over all 5 books: Beer still flags (zlib .003, tri ×2267), Cybernetics + the other 3 clear.

Plan: (a) commit fixes + CHANGELOG; (b) stop the temp watcher I started; (c) `npm.cmd run tauri build` to bake in the Rust fixes; (d) stage the fresh installer for Rab (do NOT headless-reinstall — MSIX sandbox would redirect a per-user install); (e) close + ledger. Verify: clippy `-D warnings` clean (done), py_compile (done), corpus re-verification (done). The dashboard-prototype build is a SEPARATE dedicated session (S33).
...
- ▶ Next up (user-gated): **live-verify the Assay** — Rab runs the rebuild ritual (kill widget → `npm run tauri build` → launch), confirms the `◎` station + card render, and drives the Beer flag→re-convert→re-audit loop; then the ThinkPad **exporter supersede flow** (unblocks the remedy's auto-swap — THE SUPERSEDE GAP) + enrichment wiring. Phase 5 Forgejo: still deferred, maybe never. Carry-forward: `min_chars_per_page=100` provisional — revisit after ~30 real conversions; watch the now-6-station line doesn't clip at 480 px in the live widget.
